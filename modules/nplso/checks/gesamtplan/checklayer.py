    ...
    def run(self):
        ...
        if not project_id:
            self.message_bar.pushCritical(
                    "Error", _translate("VeriSO_NPLSO_Gesamtplan", "project_id not "
                                                         "set", None))
            return

        QApplication.setOverrideCursor(Qt.WaitCursor)
        try:
            group = _translate("VeriSO_NPLSO_Gesamtplan", "Checklayer - Gesamtplan",
                               None)
            group += " (" + str(project_id) + ")"

            layer = {
                "type": "postgres",
                "title": _translate("VeriSO_NPLSO_Gesamtplan", "Beschriftung nicht io",
                                    None),
            ...
            layer = {
                "type": "postgres",
                "title": _translate("VeriSO_NPLSO_Gesamtplan", "Rechtsstatus nicht 'in Kraft'",
                                    None),
            ...
            layer = {
                "type": "postgres",
                "title": _translate("VeriSO_NPLSO_Gesamtplan", "Dokument ohne Verknüpfung",
                                    None),
